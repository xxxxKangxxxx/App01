...
| `@react-native-ml-kit/text-recognition` pod install 실패 (deployment target) | ML Kit이 iOS 15.5+ 요구하지만 프로젝트 기본 15.1 → **해결: ML Kit 제거 후 서버 사이드 OCR로 대체** (`services/ocr.ts`에서 `POST /receipt/ocr`로 이미지 전송). 서버는 `@fastify/multipart@8.x`와 `tesseract` CLI를 사용하며, 한국어 인식을 위해 `tesseract-lang` 설치 후 `OCR_LANGS="kor+eng"` 설정 필요 |
| OCR/DB/환경 설정 변경 이력 추적 | 작업 중 오류, 원인, 해결, 검증 결과는 `docs/WORK_LOG.md`에 누적 기록 |
